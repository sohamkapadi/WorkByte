...
export const jobSeekerGetAllApplications=catchAsyncError(async(req,res,next)=>{
    const {role}=req.user;
    if(role === "Employer"){
        return next(new ErrorHandler("Employer is not authorized to use this function",400));
    }
...
export const postApplication=catchAsyncError(async(req,res,next)=>{
    ...
    if(!allowedFormats.includes(resume.mimetype)){
        return next(new ErrorHandler("Resume file format not supported. Please upload in PNG,JPG or WEBP format",400));
    }
